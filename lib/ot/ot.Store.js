    ...
    init: function(store) {
      ...
      store._pubSub.on('ot', function(clientId, data) {
        ...
        if (socket = store._clientSockets[clientId]) {
          if (socket.id === data.meta.src) return;
          return socket.emit('otOp', data);
        }
      });
      return store._pubSub.on('fetch', function(out) {
        var otData, otField, otPath, otPaths, root, value, _i, _j, _len, _len2, _ref, _ref2;
        otPaths = [];
        _ref = out.data;
        for (_i = 0, _len = _ref.length; _i < _len; _i++) {
          _ref2 = _ref[_i], root = _ref2[0], value = _ref2[1];
          allOtPaths(value, root, otPaths);
        }
        if (!otPaths.length) return;
        out.ot = otData = {};
        for (_j = 0, _len2 = otPaths.length; _j < _len2; _j++) {
          otPath = otPaths[_j];
          if (otField = otFields[otPath]) otData[otPath] = otField;
        }
      });
    },
